    ...
    return (
        <>
            <div className='w-full h-screen relative flex flex-row bg-black' id='house'>
                ...
                <div className="absolute w-full h-full px-24 ">
                    <h2 className="w-full my-6 flex justify-center text-white text-3xl tracking-wider">Choose from your comfortzone</h2>
                    <div className='w-full px-6 text-white grid grid-cols-2 gap-4'>
                        {items.map((item) => (
                            <div key={item.id} className="w-[500px]">
                                <div>
                                    <Link to={`/Real-Estate-Site/unit/${item.id}`}>
                                        <img alt={item.title} className="w-[500px] h-[220px] object-cover object-center block" src={item.url} />
                                    </Link>
