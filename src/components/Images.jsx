...
const Images = () => {
  ...
  return (
    <div className='w-full h-screen flex flex-row justify-evenly items-center'>
      <Link to={`/Real-Estate-Site/type/${currentItem1.name}`} className="w-2/6 h-full bg-black cursor-pointer">
        <div className='group overflow-hidden border-2 border-black/50 relative w-full h-full bg-black'>
          ...
          {/** cursor */}
          <div className='w-20 h-20 bg-black/40 rounded-full border border-black flex justify-center items-center absolute -bottom-full left-44 group-hover:inset-y-64 transition-all duration-100 z-50'>
      ...
      <Link to={`/Real-Estate-Site/type/${currentItem2.name}`} className="w-2/6 h-full bg-black cursor-pointer">
        <div className='group overflow-hidden border-2 border-black/50 relative w-full h-full bg-black'>
          {/** overlay */}
          <div className='group-hover:bg-black/75 w-full h-full absolute z-40 transition-all duration-300'>
          </div>
      ...
      <Link to={`/Real-Estate-Site/type/${currentItem3.name}`} className="w-2/6 h-full bg-black cursor-pointer">
        <div className='group overflow-hidden border-2 border-black/50 relative w-full h-full bg-black'>
          {/** overlay */}
          <div className='group-hover:bg-black/75 w-full h-full absolute z-40 transition-all duration-300'>
          </div>
